...
class RateLimiter {
    ...
    checkRateLimit(userId) {
        ...
        if (this.isBanned(userId)) {
            ...
            return {
                allowed: false,
                message: `You are temporarily banned. Ban expires in approximately ${hours} hour(s).`,
...
function normalizePhoneNumber(phoneNumber) {
    ...
    if (digitsOnly.length < 10) {
        throw new Error('Invalid phone number format. Must contain at least 10 digits.');
    }
    
    // Return normalized format (you can adjust this format as needed)
    return digitsOnly;
}

/**
 * Uploads content to Pastebin with rate limiting and ban system based on phone number.
 * @param {string | Buffer} input - The content to upload, can be text, file path, or base64 data.
 * @param {string} phoneNumber - Phone number of the user making the request.
 ...
 */
async function uploadToPastebin(input, phoneNumber, title = 'Untitled', format = 'json', privacy = '1') {
    // Normalize phone number for consistent tracking
    let normalizedPhone;
    try {
    ...
    // Check rate limit and ban status using normalized phone number
    const rateLimitCheck = rateLimiter.checkRateLimit(normalizedPhone);
...
/**
 * Check user's current rate limit status by phone number
 * @param {string} phoneNumber - Phone number of the user
 * @returns {object} - Rate limit status information
 */
function checkUserStatus(phoneNumber) {
    const normalizedPhone = normalizePhoneNumber(phoneNumber);
    const isBanned = rateLimiter.isBanned(normalizedPhone);
    const requestCount = rateLimiter.getUserRequestCount(normalizedPhone);
    const banTimeRemaining = rateLimiter.getBanTimeRemaining(normalizedPhone);
...
/**
 * Admin function to unban a user by phone number
 * @param {string} phoneNumber - Phone number of the user to unban
 */
function unbanUser(phoneNumber) {
    const normalizedPhone = normalizePhoneNumber(phoneNumber);
    rateLimiter.unbanUser(normalizedPhone);
    console.log(`User with phone number ${phoneNumber} (${normalizedPhone}) has been unbanned.`);
}
