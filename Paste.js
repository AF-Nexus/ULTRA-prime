...
/**
 * Uploads content to Pastebin using multiple API keys with fallback, handling different input types like text, files, and base64 data.
...
async function uploadToPastebin(input, title = 'Untitled', format = 'json', privacy = '1') {
    // Dynamically import the `pastebin-api` ES module once
    ...
    for (let i = 0; i < API_KEYS.length; i++) {
        ...
        try {
            // Initialize the Pastebin client with current API key
            const client = new PasteClient(currentApiKey);
    ...
    // If we've exhausted all API keys, throw the last error
    console.error('❌ All API keys failed. Upload unsuccessful.');
    throw new Error(`Upload failed with all ${API_KEYS.length} API keys. Last error: ${lastError?.message || 'Unknown error'}`);
}

module.exports = uploadToPastebin;                '1': Publicity.Unlisted,
                '2': Publicity.Private,
            };
